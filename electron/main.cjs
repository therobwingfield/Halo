...
const AUTO = {
  ...
  knee: 0.06,          // <6% of screen bright -> leave it alone entirely
  full: 0.35,          // >=35% of screen bright -> full auto dim (a windowed PDF measures ~29%)
...
function desiredOpacity(i) {
  return Math.max(0, Math.min(0.95, Math.max(manualOpacity[i] || 0, autoOpacity[i] || 0)));
}
...
app.whenReady().then(() => {
  ...
  // Restore auto-dim preferences, then start the sampler. Active whenever Halo runs.
...
ipcMain.handle('get-autodim-state', () => ({
  ...
  displays: softwareOverlays.map((_, i) => ({
    ...
    effective: Number(Math.max(manualOpacity[i] || 0, autoOpacity[i] || 0).toFixed(4))
  }))
}));
